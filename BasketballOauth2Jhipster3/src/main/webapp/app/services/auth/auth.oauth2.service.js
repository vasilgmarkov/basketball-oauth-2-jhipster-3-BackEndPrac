    ...
    function AuthServerProvider ($http, $localStorage, Base64) {
        ...
        function login (credentials) {
            var data = 'username=' +  encodeURIComponent(credentials.username) + '&password=' +
                ...
                'client_secret=mySecretOAuthSecret&client_id=BasketballOauth2Jhipster3app';

            return $http.post('oauth/token', data, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'Accept': 'application/json',
                    'Authorization': 'Basic ' + Base64.encode('BasketballOauth2Jhipster3app' + ':' + 'mySecretOAuthSecret')
                }
            }).success(authSucess);
